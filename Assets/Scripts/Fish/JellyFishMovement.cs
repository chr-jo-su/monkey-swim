...
public class StonedFishMovement : MonoBehaviour
{
    //Variables
    ...
    private bool moveUp = false; // false == right, true == left
    ...
    void Update()
    {
        ...
        if (chasing)
        {
            ...
            spawnPos.y = transform.position.y; // updating spawnPos.y so fish don't teleport back to spawn position after losing aggro
        }
        else
        {
            ...
            transform.position = new Vector2(spawnPos.x + 0.2f * (float)Math.Sin(sineRads), transform.position.y);
            if (sineRads >= 2 * Math.PI)
                sineRads = 0;
            sineRads += 2 * Math.PI * Time.deltaTime; // one full sine wave per second

            if (transform.position.y < spawnPos.y - 8)
                moveUp = true;
